...
{
    MsgBox("Missing lnchr config file, creating default file")
    IniWrite("DefaultValue", lngui_props.configFileName, "SecureSettings", "DefaultKey")
}
else
{
    SortLNCHRConfigFile()
...
{
    if ( not FileExist("HELP-Commands.txt"))
    {
...
build_lngui() {
    ...
    lngui.Show("autosize x" . mouseX . " y" . mouseY)
}
...
lngui_edit_chng(GuiCtrlObj, Info) {
    ...
    else if (is_lngui_state("search")) ; lngui_props.query_autocomplete)
    {
        lngui_fuzzy_match_commands()
        ; lngui_autocomplete()
    }
    return
}
...
Toggle_lngui_search_state()
{
    ...
    {
        set_lngui_state("main")
        lngui._text.Value := lngui_props.title
    }
    else
    {
        set_lngui_state("search")
        lngui._text.Value := lngui_props.title "  🠖  Search Mode"
    }
}
...
CapsLock::
{
    SetCapsLockState 0 ; turn off caps lock
...
lngui_query_enter(*) { ; gui_call_sub_funcs ; When enter is triggered
    if is_lngui_state("query")
    {
    ...
    else if (is_lngui_state("search"))
    {
        if (lngui_props.commandBlockMapSearchMatch.Length == 1)
        {
            lngui._edit.value := lngui_props.commandBlockMapSearchMatch[1].ShortName
...
RegExMatchAll(haystack, needle, &matchList)
{
    matchList := []
    pos := 1
    ...
    {
        currentMatch := [match[0], match[1], Trim((match[2]))]
        matchList.Push(currentMatch)
        pos += StrLen(match[0])
    }
}

class LNCHRCommandBlock
{
    Full_IF := ""
    ShortName := ""
    Description := ""
...
FuzzyMatchCommands(commandMap, searchStr)
{
    RegExPattern := "(?i)" . RegExEscape(searchStr)
    matchedObjects := []
    for key, command in commandMap
    {
        if (RegExMatch(command.Description, RegExPattern))
        {
            matchedObjects.Push(command)
        }
    }
...
lngui_fuzzy_match_commands()
{
    tip := ""
    input := get_lngui_input() ; stash current input
    matchedObjects := FuzzyMatchCommands(lngui_props.commandBlockMap, input)

    lngui_props.commandBlockMapSearchMatch := [] ; clear the map
    for x in matchedObjects
    {
        tip .= x.Description . "`n"
        lngui_props.commandBlockMapSearchMatch.push(x)
    }
...
TryGetValueFromINIFile(keyToFind)
{
    valueToReturn := ""
    try
    {
        valueToReturn := IniRead(lngui_props.configFileName, "SecureSettings", keyToFind)
    }
    catch
    {
        MsgBox("Key " . keyToFind . " was not found")
    }
